...
### test_operator_denlist.py
This actually contains two related tests. operator_list.json contains operators and their status (denylisted, allowlisted, or deprecated). Dags are tested to see whether they comply with allowlist or denylist policies, and fail if they do not. Operators that fall outside the policy boundaries are included in the test failure output. These lists are also enforced via cluster policies in cluster_policies.py
